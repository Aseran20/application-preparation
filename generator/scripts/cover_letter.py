...
from generator.utils.config import TEMPLATES, get_output_names, COVER_LETTER_PLACEHOLDERS
...
def generate_cover_letter(content: dict, output_folder: str | Path) -> tuple[Path, Path | None]:
    ...
    if pdf_path:
        if original_pages > 1:
            print(f"[WARNING] Cover letter exceeded 1 page ({original_pages} pages) - content was trimmed!")
